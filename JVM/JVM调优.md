...
整理：内存紧凑。相当于复制算法在老年代上的简化操作，因为老年代中对象大部分都是要一直存活的对象，如果使用复制算法，需要很大的survivor-to区域
...
- ==新生代串行垃圾回收器Serail，回收算法是**复制**==
- ==老年代串行垃圾回收器SerailOld，回收算法是**标记-整理**==

**优点**

- 简单、高效，在所有垃圾回收器中内存资源消耗最少。依然是HotSpot虚拟机在**客户端模式下的默认垃圾回收器**
...
- ==新生代并行垃圾回收器ParNew，算法是**复制**==
- 除了 Serial 收集器外，只有它能与 CMS 收集器（真正意义上的并发收集器，后面会介绍到）配合工作。
  - 因为Parallel和后面的G1并没有使用HotSpot中原本设计的垃圾回收器的分代框架，技术上不兼容。
  - Parallel面向高吞吐量，CMS面向低延迟
  - JDK9之后，只有ParNew能和CMS结合使用
...
-XX:+UseParallelGC
使用 Parallel 收集器+ Parallel Old 收集器
-XX:+UseParallelOldGc
```
...
- jdk8 默认此选项
- ==新生代并行垃圾回收器Parallel Scavenge，回收算法是**复制**（和Parallel类似）==
- ==老年代并发垃圾回收器Parallel Old，回收算法是**标记-整理**==
- 开启一个，另一个也自动开启
- 垃圾回收线程个数，默认等于CPU核数
...
- ==老年代并发垃圾回收器CMS（Concurrent Mark Sweep），使用的是**标记清除算法**==。 **HotSpot 虚拟机第一款真正意义上的并发收集器**
  - 并发的垃圾回收器，和并行垃圾回收器区别在于：
    - 并行垃圾回收器，在多个垃圾回收线程运行时，不允许用户线程运行
    - 并发垃圾回收器，垃圾回收线程可以和用户线程一起并发执行，减少了STW时间
  - **存在并发失败情况（内存碎片过多），此时CMS会退化到SerialOld串行垃圾回收器，响应时间变长。即内存碎片化情况不明显时，CMS使用标记-清楚算法；内存碎片化严重时，CMS使用标记-整理算法**
  - 工作流程：
    - **初始标记**：记录下直接与GC Roots直接关联的对象，速度很快，此时会STW（单线程标记，暂停所有的其他线程）
    - **并发标记**：从GC Roots直接关联的对象开始遍历整个对象图，**此过程耗时较长，但是和用户线程并发工作**
    - **重新标记**：为了修正并发标记期间，因为用户程序继续运行而导致标记产生变动的那一部分对象的标记记录，这个阶段的停顿时间一般会比初始标记阶段的时间稍长，远远比并发标记阶段时间短，（并行标记）
    - **并发回收**：开启用户线程，同时 GC 线程开始对未标记的区域做清扫，**此过程耗时较长，但是和用户线程并发工作**
...
停顿时间模型：在指定长度为M毫秒的时间片段内，垃圾回收时间大概率不超过N毫秒。即同时注重吞吐量和低延迟，默认暂停目标时200ms。

不在专注于新生代或老年代，而是面向堆内存的任何部分

==复制+标记整理算法的局部并发垃圾回收器==

**实现原理**

- 将内存会将内存划分为多个相等的Region，每个区域都可以扮演新生代的Eden、Survivor、Old空间
- 垃圾回收时，以Region为基本单位，在后台维护一个优先级列表去记录每个Region的回收价值。
- 根据用户设定的收集停顿时间，选择最具有价值的Region进行回收。
...
- G1是针对整个内存的垃圾回收器，CMS是针对老年代的垃圾回收期
- G1选择部分最优价值的Region回收；CMS是全部老年代
  - 这是G1高吞吐量的原因
- G1回收时，步骤和CMS类似，只是最后一步采用的是并行回收，而不是并发回收。为了即使用CMS的低延迟，又有高吞吐量
...
不仅包括堆内存/非堆内存的整体信息，还可以细化到 eden 区、survivor 区等的使用情况

<img src="JVM调优.assets/image-20200723091051334.png" alt="image-20200723091051334" style="zoom:67%;" />
