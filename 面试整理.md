...
![image-20200802170347703](面试整理.assets/image-20200802170347703.png)
...
| 等待可中断     | 不可中断，除非抛出异常<br />释放锁方式：    <br />1.代码执行完，正常释放锁；    <br />2.抛出异常，由JVM退出等待 | 持有锁的线程长期不释放的时候，正在等待的线程可以选择放弃等待,(方法：<br/>1.设置超时方法 tryLock(long timeout, TimeUnit unit)，时间过了就放弃等待；<br/>2.lockInterruptibly()放代码块中，调用interrupt()方法可中断，而synchronized不行) |
...
- 两次的话，客户端不能确认服务端是否收到自己的请求

四次挥手中什么是 time_wait 状态 ？ close-wait 状态?

[参考网址](https://www.cnblogs.com/sunxucool/p/3449068.html)
...
- time_wait状态
  ...
  - **对于基于TCP的HTTP协议，关闭TCP连接的是Server端**，这样，Server端会进入TIME_WAIT状态。
- close-wait状态
  - 等待服务器主动关闭连接

time-wait状态需要等待2MSL原因：

- 一次消息传递时间是一个MSL
- 最后一次挥手，客户端向服务器发送确认信息，为了确保最后一次挥手的确认信息发送成功，需要等待。（1MSL，超过该时间，服务端会重传）
- 如果最后一次挥手发送的信息丢失，服务器在一段时间内没有收到确认信息，会再次发送给客户端关闭数据流的信息（1MSL），客户端也会再次发送确认信息。
- 所以如果2MSL内没有收到服务端发送的消息，表示信息传递成功

解决time-wait

- 允许连接被重用。HTTP协议1.1版规定default行为是Keep-Alive，也就是会重用TCP连接传输多个 request/response
- 开启处于time-wait的socket快速回收
- TIME_WAIT状态可以通过优化服务器参数得到解决，因为发生TIME_WAIT的情况是服务器自己可控的，要么就是对方连接的异常，要么就是自己没有迅速回收资源，总之不是由于自己程序错误导致的。

解决close-wait

- 代码问题，服务器再收到关闭连接之后（只结束了客户端到服务器的连接），没有主动释放连接（服务器到客户端的连接）
...
- http1.1协议中规定，表示浏览器可以继续发送消息

MAC地址是否可以相同

- 如果只有一方在使用，是没有影响的
- 如果两方同时在使用，可能会造成数据发错的情况（双方都收到，当交换机中还没有该mac地址对应的端口号，就会广播该帧、一方收到另一方）
- 如果双方在通信，通信不成功。交换机发现目的帧对应的端口就是源端口，则丢弃帧
...
【高频】MyISAM 和 InnoDB的区别    ： {是否支持行锁 、 是否支持事务 、 是否支持 MVCC 、 底层索引结构不同 }
...
- B+树相对于B树，查找效率更加稳定
- 支持顺序查找
- 磁盘读写代价更低，因为B树将数据放在节点中，因此每个盘块存储节点更少，在较坏的情况下，磁盘读写次数多
...
l count(列名)只包括列名那一列，在统计结果的时候，会忽略列值为空（这里的空不是只空[字符串](http://msd.misuland.com/pd/3545776874745501040)或者0，而是表示null）的计数，即某个字段值为NULL时，不统计。
...
- 会有影响，比较的代价提高
- 另外字段长度影响索引长度，造成索引树占据空间过大

前缀匹配

数据提交了会不会丢失

- 会存在丢失的情况，数据提交后并不会直接写到磁盘上面，而是保存在redo log
...
- 取l和r的共同最高位，将l和r同时右移直到相等，记录移位次数，然后左移该次数。
...
[参考网址](https://blog.csdn.net/shang1219178163/article/details/17120765)

八球称重：8个球中有一个球重量不同，通过天平两次找出该球

- 选择6个球放在天平两边
  - 如果相等。则称剩下两个
  - 如果不同，则在称偏重或偏轻的三个中取出两个，继续称重
    - 如果相等，则剩下一个为目标
    - 如果不等，则直接找出目标

建筑物上丢鸡蛋（2个），要求**最差情况**下扔鸡蛋的次数为最少。

- 完美负载均衡的方法应该是，扔鸡蛋1的次数加上扔鸡蛋2的次数，不论什么时候都一样，不管鸡蛋1是从哪层楼扔下时破掉的。
- 若有这种扔法，每次鸡蛋1多扔一次，鸡蛋2就可以少扔一次。
- 设初始楼层为X，当第一个鸡蛋破裂，第二个鸡蛋最坏情况投掷次数为X-1，加在一起就是1+X-1=X
- 假设第一个鸡蛋第二次破裂，此时楼层为Y，第二个鸡蛋最坏情况投掷次数为Y-1-X，加在一起就是Y-1-X+2=Y-X+1。
- 使Y-X+1=X，则Y=X +(X-1)，即每次增加楼层-1即可。
...
- 首先查看目的ip与主机是否在一个网段
- 是。查看ARP缓存，获取目的ip-mac映射。
  - 如果没有，就广播获取；有，设置源MAC和目的MAC
- 不是。查看ARP缓存，获取默认网关的ip-mac映射。
  - 发送给默认路由器
  - 路由器通过路由表检查目的ip的网段，然后找到下一跳地址。

mac地址是可以冲突的，只要不是在同一个局域网下面就行
...
- avtivity和service通信，service的bind方法会返回一个Bind()对象，activity中重写一个ServiceConnection对象，然后通过bindService()方法绑定Service。绑定成功，此时Service会先onCreate，然后将bind()方法结果返回给ServiceConnection的回调函数。解绑后，service也会触发onDestroy()方法

handler机制

- [参考网址](https://blog.csdn.net/bboyfeiyu/article/details/38555547?utm_medium=distribute.pc_relevant_t0.none-task-blog-BlogCommendFromMachineLearnPai2-1.channel_param&depth_1-utm_source=distribute.pc_relevant_t0.none-task-blog-BlogCommendFromMachineLearnPai2-1.channel_param)

[Android源码参考](https://blog.csdn.net/luoshengyang/article/details/6747696)

[面试1](https://www.nowcoder.com/discuss/461587?channel=1009&source_id=home_feed)

[面试2](https://www.nowcoder.com/discuss/454446?channel=1009&source_id=discuss_terminal_discuss_history)
...
- [参考网址](https://www.cnblogs.com/heyanan/p/10109338.html)
- URL的编码格式采用的是ASCII码而不是Unicode，这也就是说你不能在URL中包含任何非ASCII字符
- URL参数字符串中如果包含”&”或者”%”势必会造成服务器解析错误，所以需要对其进行编码。 

安卓handler机制

kotlin let with run apply 方法使用

sharedpreferences apply 和commit区别

threadlocl作用

版本号比较

- 版本号样式为11.23.21或者12.23.23.1，其中对于11.23.21和11.23.21.0可以认为是相等的
- 大于返回1，等于返回0，小于返回-1，异常返回-100

字符串移位

1-1024乱序排列，向其中插入x，x∈[1,0124]，如何快速找到x
